    ...
    def learn(self):
        ...
        for idx, e in enumerate(user_idxs):
            ...
            while t < self.config.episode_length:
                ...
                rec_item_idx = torch.topk(ranking_scores.flatten(), 5).indices #torch.argmax(ranking_scores)
                rec_item_emb = candidate_items[rec_item_idx]

                # Get item reward
                reward = 0
                for idx in rec_item_idx:
                    if idx in user_reviews[:, i]:
                        # Reward from rating in dataset if item rated by user
                        reward += user_reviews[user_reviews[:, i] == idx, self.r][0]
                    else:
                        ...
                        with torch.no_grad():
                            if self.config.zero_reward:
                                ...
                                reward += self.reward_function(torch.tensor(e).to(self.device), idx)
                reward = reward/len(rec_item_idx)
                ...
                if reward > 0:
                    ...
                    history_buffer.push(rec_item_emb[0].detach().clone())
                    # Observe next state
                    with torch.no_grad():
    ...
    def online_evaluate(self):
        ...
        for idx, e in enumerate(user_idxs):
            ...
            while t < self.config.episode_length:
                ...
                reward = 0
                for idx in rec_item_idx:
                    if idx in user_reviews[:, self.i]:
                        # Reward from rating in dataset if item rated by user
                        reward += user_reviews[user_reviews[:, self.i] == idx, self.r][0]
                    else:
                        # Item not rated by user, reward from PMF
                        with torch.no_grad():
                            reward += self.reward_function(torch.tensor(e).to(self.device), idx)
                ...
                if reward > 0:
                    # Update history buffer with new item
                    history_buffer.push(rec_item_emb[0].detach().clone()) # still use first recommended one
                    # Observe next state
                    with torch.no_grad():
                        next_state = self.state_rep_net(user_emb, torch.stack(history_buffer.to_list()))
                else:
    ...
    def offline_evaluate(self, T):
        ...
        for idx, e in enumerate(user_idxs):
            ...
            while t < T:
                ...
                # Get recommended item
                rec_item_idx = torch.topk(ranking_scores[user_reviews[:, self.i].int()].flatten(), 5).indices#torch.argmax(ranking_scores[user_reviews[:, self.i].int()])
                rec_item_emb = user_candidate_items[rec_item_idx]

                # Get item reward
                reward = 0
                for idx in rec_item_idx:
                    reward += user_reviews[idx, self.r]
                reward = reward / len(rec_item_idx)
